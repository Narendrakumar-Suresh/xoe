...
class CrossEntropyLoss:
    ...
    def __call__(self, logits: Tensor, targets: Tensor) -> Tensor:
        ...
        if jnp.issubdtype(targets._data.dtype, jnp.integer) or targets._data.ndim < logits._data.ndim:
            if logits._data.ndim > 1:
                batch_size = logits._data.shape[0]
                loss = -log_softmax[jnp.arange(batch_size), targets._data.astype(jnp.int32)]
            else:
